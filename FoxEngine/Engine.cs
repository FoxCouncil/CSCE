...
{
    private readonly int[] _fpsAvgBuffer = new int[8];

    private readonly Thread _engineThread;

    private readonly Sound _engineSound;

    private readonly Sprite _defaultDrawTarget;
    ...
    public Dictionary<KeyboardButton, Button> Keyboard = new();

    public Dictionary<MouseButton, Button> Mouse = new();
...
#pragma warning restore IDE0059 // Unnecessary assignment of a value
    ...
    void Run(object obj)
    {
        ...
        while (IsRunning)
        {
            ...
            //oy = 128 - (int)(SampleSquareWave(_frequency / 440.0f, 1f) * 32f) - 64;
            
            //for (var t = 1f; t < 2f; t += 1f / 128f)
            //{
            //    var sample = SampleSquareWave(_frequency / 440f, t);

            //    y = (128 - (int)(sample * 32f)) - 48;
            //    x = (int)((t - 1f) * 128f) + 64;

            //    DrawLine(ox, oy, x, y, Pixel.White);
    ...
    //float SampleSquareWave(float f, float t)
    //{
    //    float a = 0, b = 0, p = (float)(_dutyCycle * 2f * Math.PI);

    //    for (var n = 1f; n < _harmonics; n++)
    //    {
    //        var c = (float)(n * f * 2.0 * Math.PI * t);
    //        a += (float)(Math.Sin(c) / n);
    //        b += (float)(Math.Sin(c - p * n) / n);
    //    }

    //    return (float)(2f / Math.PI * (a - b));
    //}
